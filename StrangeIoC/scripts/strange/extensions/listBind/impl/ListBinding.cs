...
namespace strange.extensions.listBind.impl
{
    public class ListBinding : Binding, IListBinding
    {
        ...
        public new IListItemBinding To(object type)
        {
            Type typeObj = type as Type;
            if (typeObj == null) throw new Exception("Key must be of type 'Type'");

            var injectionBinding = injectionBinder.GetBinding(type, null);
            if (injectionBinding == null)
            {
                injectionBinding = new InjectionBinding(resolver);
                injectionBinding.Bind(type);
            }

            ToTypeBinding toType = new ToTypeBinding(injectionBinding, ListItemType, typeObj);
